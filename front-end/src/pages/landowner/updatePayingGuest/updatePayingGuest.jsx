...
import { getSinglePayingGuest, updatePayingGuest } from "../../../api/payingGuest/payingGuest";

function UpdatePayingGuests() {
  ...
  useEffect(() => {
    async function getSinglePayingGuests() {
      ...
      if (response.status === 200) {
        ...
        setFloorNumber(response.data.data.floorNumber);
...
const handleDeleteContract = () => {
  ...
  const UpdateProperty = async () => {
    ...
    if (contractPaper instanceof File) {
      ...
      const response = await updatePayingGuest(id,data);
      
    setLoading(true);
    if (response.status === 200) {
      setLoading(false);
      navigate("/landowner/my_paying_guests");
      toast.success("Paying guests Updated successfully");
  ...
  return (
    <div className="flex flex-col items-center h-screen px-4 pt-20 pb-4">
      <div className="w-[60%] p-6 bg-white">
        <h2 className="mb-4 text-2xl font-bold">Update Paying Guests</h2>
        <div className="mb-6">
...
export default UpdatePayingGuests;
