...
import { getSingleService, updateService } from "../../../api/service/Service";
import { useNavigate, useParams } from "react-router-dom";
import { toast } from "react-toastify";
function UpdateServiceByAdmin() {
  const { id } = useParams();
  ...
  useEffect(() => {
    const getOne = async () => {
      ...
      if (response.status === 200) {
        ...
        setImage(fetchedService.thumbnail);
        setDescription(fetchedService.description);
        setTitle(fetchedService.title);
        setSubServices(fetchedService.subServices);
      }
    };
    getOne();
  }, [id]);
  ...
  const handleThumbnailUpload = (index, file) => {
    const updatedSubServices = subServices.map((subService, i) =>
      i === index ? { ...subService, thumbnail: file } : subService
    );
    setSubServices(updatedSubServices);
  };
  ...
  return (
    <div className="flex flex-col items-center min-h-screen px-4 pt-20 pb-4">
      <div className="w-[60%] p-6 bg-white">
        <h2 className="mb-4 text-2xl font-bold">Update Your Service</h2>
        ...
        <div className="w-full ">
          {image && (
            <div className="w-full  overflow-hidden rounded-lg h-[16rem]">
                ...
                {subService.thumbnail && (
                  <div className="w-full h-full overflow-hidden rounded-lg">
                    <div className="relative w-max">
                      <img
                        src={
                          subService.thumbnail instanceof File
                            ? URL.createObjectURL(subService.thumbnail)
                            : subService.thumbnail
                        }
                        alt="Uploaded"
...
export default UpdateServiceByAdmin;
