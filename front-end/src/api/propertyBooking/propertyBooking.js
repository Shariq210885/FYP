...
const UpdatePropertyBooking = async (session_id) => {
    try {
      ...
      return await instance.get(`propertybooking/success?session_id=${session_id}`);
    } catch (error) {
      return error;
  }
};
...
const UpdateOnePropertyBooking = async (PropertId,data) => {
  try {
    ...
    return await instance.patch(`propertybooking/${PropertId}`,data,{headers});
  } catch (error) {
    return error;
  }
}; 

export {createPropertyBooking,UpdatePropertyBooking,myPropertyBookings,UpdateOnePropertyBooking}